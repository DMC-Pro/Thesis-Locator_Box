	...
	if($result == false){
		...
		while($row = $result->fetch_assoc()) {	
			...
			if($r == false){
				...
				echo '<table>
						<tr>
							<td colspan="4" class="bb">Device '.$row["device_loc_id"].'<button onclick="newCenter('.$row["device_latitude"].','.$row["device_longitude"].')" class="dropbtn2" id="dev'.$count.'">Show on Map</button></td>
						</tr>
						<tr>
							<td class="aa">Lat:</td>
